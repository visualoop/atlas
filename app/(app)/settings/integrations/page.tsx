...
export default function IntegrationsPage() {
  const byCategory = PROVIDERS.reduce<Record<string, typeof PROVIDERS[number][]>>((acc, p) => {
  ...
  return (
    <div className="space-y-12">
      <p className="text-sm text-muted-foreground max-w-prose">
        Org-level keys. Encrypted at rest in Convex with the master encryption key. Only org
        owners can view; members use them via Atlas server (never in the browser). See{" "}
        <code className="font-mono text-xs">plan/06-auth-and-permissions.md</code>.
      </p>
      ...
      <p className="text-xs text-muted-foreground italic">
        ...
        phase for each non-AI provider. The encryption layer + audit logging is ready.
      </p>
    </div>
  );
}
